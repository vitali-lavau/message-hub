<template>
    <div class="file-uploader flex items-start" @dragover.prevent @drop.prevent="handleDrop">
        <input
            ref="fileInput"
            type="file"
            multiple
        ...
        />
        <p v-if="errorMessage" class="file-uploader__error" role="alert">
            {{ errorMessage }}
        </p>
        <p v-else-if="files.length" class="file-uploader__hint">
            PNG, JPG, PDF, DOCX or PPTX · up to 10 MB each
        </p>
        ...
        <div v-if="files.length" class="file-uploader__previews flex flex-wrap">
            <div
            ...
            >
                <img
                    v-if="file.type.startsWith('image/') && file.previewUrl"
                    :src="file.previewUrl"
                    :alt="`Preview of ${file.name}`"
                    class="file-img img-responsive"
                />
                <div v-else class="file-icon flex items-center justify-center">
                    {{ getAttachmentTypeLabel(file.name, file.type) }}
                </div>
...
import {
    ...
    getFileTypeLabel as getAttachmentTypeLabel,
} from '~/utils/formatFileSize';
import {
    ATTACHMENT_ACCEPT,
    MAX_ATTACHMENT_SIZE,
    SUPPORTED_ATTACHMENT_TYPES,
} from '~/utils/demoLimits';
...
const emit = defineEmits<{
    (event: 'filesChanged', files: Attachment[]): void;
}>();
...
function processFiles(selectedFiles: File[]) {
    ...
    selectedFiles.forEach((file) => {
        if (file.size <= MAX_ATTACHMENT_SIZE && isSupportedFileType(file)) {
            if (isImage(file)) {
            ...
            errorMessage.value = `${file.name} exceeds the 10 MB limit or is not a supported file type.`;
        }
    });
}
...
function isSupportedFileType(file: File): boolean {
    return SUPPORTED_ATTACHMENT_TYPES.includes(
        file.type as (typeof SUPPORTED_ATTACHMENT_TYPES)[number]
    );
}
