<template>
    <button
        ...
        :class="{
            ...
            'direct-message-item--selected': props.isSelected,
        }"
        :aria-current="props.isActive ? 'true' : undefined"
        :aria-pressed="props.selectable ? Boolean(props.isSelected) : undefined"
        :aria-label="`${name}${unreadCount ? `, ${unreadCount} unread` : ''}`"
        @click="emit('select', props.id)"
    >
        <Avatar :name="name" :image-url="avatarUrl" :status="status" />
        <div class="direct-message-item__name">
            <span class="direct-message-item__header">
                <span class="direct-message-item__label">{{ name }}</span>
                <time v-if="lastMessageTime" class="direct-message-item__time">
                    {{ lastMessageTime }}
                </time>
                <span v-if="unreadCount" class="direct-message-item__unread">
                    {{ unreadCount }}
                </span>
            </span>
            <span v-if="subtitle" class="direct-message-item__subtitle">{{ subtitle }}</span>

            <div v-if="selectable" class="direct-message-item__icon">
                <svg
                    width="20"
                    height="20"
                    viewBox="0 0 20 20"
